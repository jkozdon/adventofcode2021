...
def parta(name, input):
    uniq = (2, 3, 4, 7)
    v = 0
    for item in input:
        msg = item.split("|")[1]
        v += sum([1 if len(t) in uniq else 0 for t in msg.split(" ")])
    print(name, ":", v)
...
# E f
#  G
def partb(name, input):
    total = 0
    for item in input:
        d = defaultdict(list)
        for k in item.split("|")[0].split(" "):
            d[len(k)].append(k)
        seven = d[3][0]
        one = d[2][0]
        four = d[4][0]
        ...
        # look for 2 and eliminate known info
        #  A
        #   c
        #  d
        # E  
